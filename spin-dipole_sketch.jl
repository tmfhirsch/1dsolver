...
function my_scheme(α_::α, α::α)
    ...
    return Ωsum*15/(2*S_+1)*sqrt((2l_+1)/(2l+1))
end
...
# shows C=0 cancels ̂S₁⋅̂S₂ term, and C=1,C=3,C=4 terms are zero
function brute_simplifier(lmax=4::Int, tol=1e-10)
    function C4(α_::α, α::α)
        #unpack quantum numbers
        S₁_,S₂_,S_,mS_,l_,ml_=α_.S₁,α_.S₂,α_.S,α_.mS,α_.l,α_.ml
        S₁, S₂, S, mS, l, ml = α.S₁, α.S₂, α.S, α.mS, α.l, α.ml
    ...
    function C0_minus_diag(α_::α,α::α)
        #unpack quantum numbers
        S₁_,S₂_,S_,mS_,l_,ml_=α_.S₁,α_.S₂,α_.S,α_.mS,α_.l,α_.ml
        S₁, S₂, S, mS, l, ml = α.S₁, α.S₂, α.S, α.mS, α.l, α.ml
                        ...
                        for mS_ in -S_:1:S_
                            for l_ in 0:1:lmax
                                for ml_ in -l_:1:l_
                                    ...
                                    # test and compare
                                    if C4(bra,ket) > tol
                                        return "C4 fail for: ", bra, ket
                                    end
                                    if C3(bra,ket) > tol
                                        return "C3 fail for: ", bra, ket
                                    end
                                    if C1(bra,ket) > tol
                                        return "C1 fail for: ", bra, ket
                                    end
                                    if abs(C0_minus_diag(bra,ket)) > tol
                                        return "C0 fal for: ", bra, ket
                                    end
                                end
                            end
                        end
    ...
    println("C=1,3,4 terms are all zero and C=0 term cancels ̂S₁⋅ ̂S₂ term")
end
